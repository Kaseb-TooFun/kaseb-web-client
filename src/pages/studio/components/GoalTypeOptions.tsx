...
const AllGoalTypeOptions = (props: GoalTypeOptionsProps) => {
	...
	const goalTypes = goalTypeOptions.map((goalData) => {
		...
		if (primaryGoalType === 'page_visit') {
			...
			const onSwitchChange = (value: boolean) => {
				if (value) {
					if (goalSelector) postMessageToIframe('disable-inspector');
		...
		const goalForm = (
			<>
				...
				<Button
					...
					onClick={() => {
						const goalFieldName = document.getElementById(
							'goal_form_name'
						) as HTMLInputElement;
						...
						if (primaryGoalType === 'page_visit') {
							// save goal link
							const goalFieldLink = document.getElementById(
								'goal_form_link'
							) as HTMLInputElement;
				...
				>
					<span style={{ color: 'white' }}>ذخیره و ادامه</span>
				</Button>
			</>
		);
		...
		const isSelected =
			...
			(!goalType && primaryGoalType === goalData.name);
					...
					<div className="goal-option-header">
						{goalData.img ? (
							...
							<div className="goal-option-header-icon">
								<i
									className={`big ${goalData.icon} icon`}
									style={{
										color: !isSelected ? '#af9b18' : 'white'
									}}
								/>
							</div>
						) : null}
	...
	return <div className="goal-option-all">{goalTypes}</div>;
};
