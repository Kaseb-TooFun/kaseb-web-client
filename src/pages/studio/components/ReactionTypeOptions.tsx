...
const modalOptions = [{ name: 'modal', showName: 'مُدال' }];
...
const ReactionOption = (props: ReactionOptionProps) => {
	...
	const isSelected =
		Boolean(reactionName) && reactionName === reactionData.name;
	return (
			...
			>
				<span
					...
					style={{
						fontSize: '15px',
						fontWeight:
							reactionName === reactionData.name
								? 'bold'
								: 'normal',
...
const AllReactionTypeOptions = (props: AllReactionTypeProps) => {
	...
	if (hasContentCard) {
		contentCardDivs = (
			<>
				<Divider style={{ color: '#af9b18' }}>لایه محتوایی</Divider>
