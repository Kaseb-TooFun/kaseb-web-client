...
import AllGoalTypeOptions from 'src/pages/studio/components/GoalTypeOptions';
...
const AddEditReaction = (props: IProps) => {
	...
	if (
		configInitialData.values.urlPatterns &&
		configInitialData.values.urlPatterns.length > 0
	) {
	...
	const [data, setData] = useState({
		...
		btnTextColor:
			configInitialData.values.btnTextColor || defaultBtnTextColor,
		opacity: configInitialData.values.opacity || '1',
		fontFamily:
			configInitialData.values.fontFamily || persianAvailableFonts[0].key,
	...
	const onFormSubmit = () => {
		if (isDemo) {
			...
			let cleanedUrlPatterns;
			if (urlPatterns && urlPatterns.length > 0) {
				cleanedUrlPatterns = urlPatterns.filter(
					(s: string) => s !== ''
				);
			}
				...
				Api.config
					...
					.then((response) => {
						if (response.status == 200) {
							...
							const cid = response.data['configs'][0].id;
							setConfigID(cid);
							// fetchConfigList();
	...
	const colorsDiv = (
		<>
			<div className="my-input-label">{'رنگ پس زمینه'}</div>
			...
			<div className="my-input-label">{'رنگ عنوان'}</div>
			...
			<div className="my-input-label">{'رنگ پس‌زمینه دکمه'}</div>
			<Form.Item label={null} name={'btnColor'}>
				<Input
					className="my-input"
					placeholder={defaultBtnColor}
					maxLength={64}
				/>
			</Form.Item>
			<ColorSelector
			...
			<div className="my-input-label">{'رنگ متن دکمه'}</div>
			<Form.Item label={null} name={'btnTextColor'}>
				<Input
					className="my-input"
					placeholder={defaultBtnTextColor}
					maxLength={64}
				/>
			</Form.Item>
			<ColorSelector
	...
	if (reactionType === 'banner' || reactionType === 'modal') {
		reactionDetails = (
			<div className="base-color" dir="rtl">
				<h2 className="form-header">محتوا</h2>
				<p className="form-message">
					لطفا محتوای مورد نظر خود که در لایه محتوایی نمایش داده
					می‌شود یا از کاربر پرسیده می‌شود را تنظیم کنید
				</p>

				<div className="my-input-label">عنوان محتوا</div>
				...
				<div className="my-input-label">متن</div>
				...
				<Divider style={{ color: darkBaseColor }}>رنگ</Divider>

				{colorsDiv}

				<div className="my-input-label">فونت محتوا</div>
				...
				<div className="my-input-label">جهت متن؟</div>
				<Form.Item label={null} name="isRTL" valuePropName="checked">
					<Switch
						onChange={onFormChange}
						defaultChecked={true}
				...
				<div className="my-input-label">قابل بسته شدن؟</div>
				<Form.Item
					label={null}
					name="isCloseable"
				...
				<Form.Item label={null} name="showOnce" valuePropName="checked">
					<Switch
						onChange={onFormChange}
						defaultChecked={true}
		...
		reactionDetails = (
			<div className="base-color" dir="rtl">
				...
				<div className="my-input-label">المان مقصد</div>
				<div>
					<DestInspector
						destSelector={destSelector}
		...
		<>
			<details>
				...
				<p className="form-message" style={{ fontSize: '1.1em' }}>
					در این قسمت می‌توانید آدرس صفحاتی را که می‌خواهید واکنش در
					آن‌ها اجرا شود تعیین نمایید.
				</p>
				<p className="form-message" style={{ fontSize: '1em' }}>
					برای این محدودیت‌ها می‌توانید از عبارات باقاعده (RegEx)
					استفاده نمایید.
				</p>
				<p className="form-message" style={{ fontSize: '1em' }}>
					در صورتی که هیچ محدودیتی تعریف نکنید واکنش در همه صفحات اجرا
					خواهد شد.
				</p>
				<UrlPattersInputs
	...
	const steps = [
		...
		},
		{
			...
			content: (
				<div className="base-color" dir="rtl">
					...
					<p className="form-message">نوع واکنش خود را انتخاب کنید</p>
					<p className="form-message">
						اگر نوع واکنش شما از نوع انیمیشن روی المان می‌باشد نیاز
						به تعریف کارت محتوا نیست
					</p>
		...
		},
		{
			...
			title: '',
			showName: !(reactionType === 'banner' || reactionType === 'modal')
				? 'استایل'
				: 'محتوا',
